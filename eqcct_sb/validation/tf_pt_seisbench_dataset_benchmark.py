...
Example (quick smoke test, CPU only):

  PYTHONPATH=/path/to/EQCCT_to_Seisbench python -m eqcct_sb.validation.tf_pt_seisbench_dataset_benchmark \\
    --datasets txed --max-windows 200 --profiles cpu

Example (TXED + STEAD, all three profiles, cap 50k windows per dataset):

  PYTHONPATH=. python -m eqcct_sb.validation.tf_pt_seisbench_dataset_benchmark \\
...
def build_child_command(
    *,
    ...
    cmd: list[str] = [
        sys.executable,
        "-m",
        "eqcct_sb.validation.tf_pt_seisbench_dataset_benchmark",
        "--repo",
        str(repo),
        "--p-h5",
        str(p_h5),
